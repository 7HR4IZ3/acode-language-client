...
export class LanguageProvider {
  ...
  $registerEditor(editor: Ace.Editor) {
    ...
    editor.on("changeSession", ({ session }) =>
      this.$registerSession(session, editor)
    );
      ...
      editor.on("changeSelection", () => {
        if (!$timer)
          $timer = setTimeout(() => {
            ...
            let sessionLanguageProvider = this.$getSessionLanguageProvider(
              editor.session
            );

            this.$messageController.findDocumentHighlights(
              this.$getFileName(editor.session),
              fromPoint(cursor),
              sessionLanguageProvider.$applyDocumentHighlight
            );
            $timer = undefined;
          }, 50);
      });
      ...
      this.doHover(session, docPos, (hover) => {
        ...
        var errorMarker =
          this.$getSessionLanguageProvider(
            session
          ).state?.diagnosticMarkers?.getMarkerAtPosition(docPos);
...
.language_highlight_write {
  ...
  setSessionOptions<OptionsType extends ServiceOptions>(
    session: Ace.EditSession,
    options: OptionsType
  ) {
    let sessionLanguageProvider = this.$getSessionLanguageProvider(session);
  ...
  setGlobalOptions<T extends keyof ServiceOptionsMap>(
    serviceName: T & string,
    options: ServiceOptionsMap[T],
    merge = false
  ) {
  ...
  doHover(
    session: Ace.EditSession,
    position: Ace.Point,
    callback?: (hover: Tooltip | undefined) => void
  ) {
  ...
  provideSignatureHelp(
    session: Ace.EditSession,
    position: Ace.Point,
    callback?: (signatureHelp: Tooltip | undefined) => void
  ) {
  ...
  getTooltipText(hover: Tooltip): string {
    return hover.content.type === "markdown"
      ? CommonConverter.cleanHtml(
          this.options.markdownConverter!.makeHtml(hover.content.text)
        )
      : hover.content.text;
  }
  ...
  doComplete(
    editor: Ace.Editor,
    session: Ace.EditSession,
    callback: (CompletionList: Ace.Completion[] | null) => void
  ) {
    let cursor = editor.getCursorPosition();
  ...
  doResolve(
    item: Ace.Completion,
    callback: (completionItem: lsp.CompletionItem | null) => void
  ) {
    this.$messageController.doResolve(
      item["fileName"],
  ...
  $registerCompleters(editor: Ace.Editor) {
    let completer = {
      ...
      getDocTooltip: (item: Ace.Completion) => {
        if (
          this.options.functionality.completionResolve &&
          !item["isResolved"] &&
          item.completerId === completer.id
        ) {
          this.doResolve(item, (completionItem?) => {
            ...
            if (completion.docHTML) {
              ...
              item.docHTML = CommonConverter.cleanHtml(
                this.options.markdownConverter!.makeHtml(
                  completion["docMarkdown"]
                )
              );
            }
            if (editor["completer"]) {
...
class SessionLanguageProvider {
  ...
  private $changeMode = () => {
    ...
    this.$deltaQueue = [];
    this.$messageController.changeMode(
      this.fileName,
  ...
  $applyDocumentHighlight = (documentHighlights: lsp.DocumentHighlight[]) => {
    ...
    if (documentHighlights) {
      ...
      this.state.occurrenceMarkers.setMarkers(
        fromDocumentHighlights(documentHighlights)
      );
    }
  };
