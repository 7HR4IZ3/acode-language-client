...
type Validation = {
  (
    ...
    servicesInstances?: LanguageService[]
  ): Promise<void>;
  (
    document: TextDocumentIdentifier | undefined,
    servicesInstances: LanguageService[]
  ): Promise<void>;
};
...
export class ServiceManager {
  ...
  constructor(ctx: { postMessage; addEventListener }) {
    ...
    let doValidation: Validation = async (
      document?: TextDocumentIdentifier,
      servicesInstances?: LanguageService[]
    ) => {
  ...
  private static async $initServiceInstance(
    service: ServiceConfig | LanguageClientConfig,
    ctx
  ...
  private async $getServicesInstancesByMode(
    mode: string
  ): Promise<LanguageService[]> {
